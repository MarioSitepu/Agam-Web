...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#111111] py-4 sm:py-5 px-4 sm:px-6 lg:px-12 transition-all duration-300">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        {/* Logo Section */}
        <Link href="/" className="flex items-center gap-3 relative z-20">
          <div className="w-8 h-8 rounded-full bg-[#FFB800] flex items-center justify-center flex-shrink-0">
             <span className="text-black font-bold text-lg leading-none mt-1">W</span>
          </div>
          <div className="flex items-center">
            <span className="text-sm md:text-xl lg:text-2xl font-bold text-white uppercase tracking-wider font-sans">WARKOP <span className="text-[#FFB800] italic font-script ml-0.5 sm:ml-1 normal-case tracking-normal text-lg md:text-2xl lg:text-3xl">Agam</span></span>
          </div>
        </Link>
        ...
        <div className="hidden lg:flex items-center space-x-12 absolute left-1/2 -translate-x-1/2">
          ...
          >
            ...
            {/* Desktop Mega Menu */}
            <div className={`
              fixed top-[65px] sm:top-[72px] lg:top-[80px] left-0 right-0 bg-[#1A1A1A] border-t border-white/10 shadow-2xl transition-all duration-300 overflow-hidden
            ...
            `}>
              <div className="max-w-7xl mx-auto grid grid-cols-4 h-full min-h-[300px] lg:min-h-[400px]">
                {/* Categories List */}
                <div className="col-span-1 border-r border-white/5 py-6 lg:py-8 px-3 sm:px-4 lg:pr-4">
                  {menuData.map((cat) => (
                    <button
                      key={cat.id}
                      onMouseEnter={() => setActiveCategory(cat.id)}
                      className={`w-full text-left px-3 sm:px-4 lg:px-6 py-2 lg:py-3 rounded-xl transition-all flex items-center justify-between group text-sm sm:text-base ${
                ...
                <div className="col-span-3 p-4 sm:p-6 lg:p-8 overflow-y-auto max-h-[400px] lg:max-h-[500px] scrollbar-thin scrollbar-thumb-white/10">
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
                    {activeCategoryData?.items.map((item, idx) => (
                      <div key={idx} className="group cursor-pointer">
                        ...
                        <h4 className="text-white font-bold text-xs sm:text-sm uppercase tracking-tight group-hover:text-[#FFB800] transition-colors line-clamp-2">{item.name}</h4>
      ...
      {/* Mobile Menu Drawer */}
      <div className={`
        lg:hidden absolute top-full left-0 right-0 bg-[#111111] border-t border-white/10 shadow-2xl transition-all duration-300 ease-in-out max-h-[calc(100vh-65px)] sm:max-h-[calc(100vh-72px)]
        ${isOpen ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-4 pointer-events-none"}
      `}>
        <div className="flex flex-col p-4 sm:p-6 space-y-3 sm:space-y-4 overflow-y-auto max-h-[calc(100vh-65px)] sm:max-h-[calc(100vh-72px)]">
          <Link href="/" className="text-lg sm:text-xl font-bold text-white" onClick={() => setIsOpen(false)}>Home</Link>
          
          <Link href="/order" className="text-lg sm:text-xl font-bold text-white hover:text-[#FFB800] transition-colors" onClick={() => setIsOpen(false)}>Order</Link>
          
          <div className="pt-3 sm:pt-4 border-t border-white/5">
             <p className="text-[#FFB800] text-xs sm:text-sm font-bold mb-3 sm:mb-4 uppercase tracking-widest">Our Menus</p>
             <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:gap-4">
                {menuData.slice(0, 4).map((cat) => (
                  ...
                  >
                    ...
                    <span className="text-white text-xs font-bold uppercase line-clamp-2">{cat.name}</span>
                  </Link>
                ))}
             </div>
          </div>
          ...
          <div className="flex items-center gap-4 sm:gap-6 pt-4 sm:pt-6 border-t border-white/10">
            ...
            <button className="text-[#FFB800] text-sm sm:text-base font-medium flex items-center gap-2" onClick={() => setIsOpen(false)}>
