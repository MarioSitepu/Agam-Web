      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 md:gap-10">
          {activeCategory?.items.map((item, index) => (
            <div 
              key={index} 
              className="bg-gray-50 rounded-[1.5rem] xs:rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6 flex flex-col group relative transition-all hover:bg-white hover:shadow-2xl hover:shadow-primary-brown/10 border border-transparent hover:border-primary-brown/5"
            >
              {/* Product Image */}
              <div className="relative w-full aspect-square mb-4 sm:mb-6 overflow-hidden rounded-xl xs:rounded-[1.5rem] sm:rounded-[2rem] shadow-lg">
                <Image
              ...
              <div className="w-full flex-grow flex flex-col">
                <div className="flex justify-between items-start mb-2">
                   <h3 className="text-lg xs:text-xl font-bold text-gray-900 group-hover:text-primary-brown transition-colors uppercase leading-[1.2] max-w-[80%]">
                    {item.name}
                  </h3>
                  <div className="flex items-center gap-0.5 text-[#FFB800]">
                    <Star className="w-3 h-3 xs:w-3.5 xs:h-3.5 sm:w-4 sm:h-4 fill-[#FFB800]" />
                    <span className="text-[10px] xs:text-xs font-bold text-gray-600">4.8</span>
                  </div>
                </div>
